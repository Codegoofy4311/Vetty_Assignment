...
GROUP BY 1
ORDER BY 1;
-- This gives number of successful purchases for each calendar month.
...
-- The final result is one number — that how many stores are in  5 or more orders in Oct-2020.
...
-- For each store, this shows the fastest refund turnaround in minutes.
...
FROM transactions
ORDER BY store_id, purchase_time ASC;  -- pick the earliest purchase_time for each store

-- Shows how much the first order for each store was worth.
...
-- Step 3: Count which item_name appears most among those first purchases.
SELECT i.item_name
FROM (
...
--  This returns the single item name that appears most often on buyers' first orders.
...
-- If refund was made within 72 hours of the purchase, refund_processable is TRUE.



-- Q7: Create a rank by buyer_id and filter for only the second purchase per buyer (ignore refunds)
-- We compute a rank (1,2,3...) for each buyer based on purchase_time,
...
-- This returns each buyer's second non-refunded purchase (if they have one).
...
-- This returns the timestamp of each buyer's second purchase without using min or max.
